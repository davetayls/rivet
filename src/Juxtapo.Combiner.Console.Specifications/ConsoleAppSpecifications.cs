...
namespace Juxtapo.Combiner.Console.Specifications
{
	public class ConsoleAppSpecifications
	{
		...
		public void RunSpecifications()
		{
			...
			"Run() returns 1 when null is passed".Assert(() => ConsoleApp.Run(null).ShouldBe(false));
			"Run() returns 1 when empty string[] is passed".Assert(() => ConsoleApp.Run(new string[] {}).ShouldBe(false));
			"Run() returns 0 when when array with 1 string is passed".Assert(() => ConsoleApp.Run(new[] {""}).ShouldBe(true));
		}
	}
}